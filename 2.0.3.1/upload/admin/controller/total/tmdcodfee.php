...
//lib
class ControllerTotalTmdcodfee extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('total/tmdcodfee', 'token=' . $this->session->data['token'] . '&type=module', true));
			}
			else{
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('total/tmdcodfee', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('total/tmdcodfee', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->data['token'], 'SSL');

		$data['staysave'] = $this->url->link('total/tmdcodfee', '&status=1&token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('total/tmdcodfee.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'total/tmdcodfee')) {
